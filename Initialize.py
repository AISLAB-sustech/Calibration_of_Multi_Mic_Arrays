...
def DOA_ERROR(DOAS,DOA_STD):
    '''
    :param DOAS: Theoretical doa vector
    :param DOA_STD: std of Azimuth and Elevation angles
    :return: DOA value affected by noise
    '''
...
def get_initial_value(dataset,category):
    ...
    # Not the preset simulation trajactory
    if dataset != 0:
        tdoa_mea, doa_mea, odo_mea = load_measurement(dataset,type = category)
        # translate displacement to global frame
    ...
    # Estimated values of all unkown parameters
    x = np.zeros((8*mic_num+3*time_steps,1))
    for i in range(1,mic_num):
        # For Experiment 1: x = x_gt + Gaussion Noise
        # x[i*8:i*8+3]   = x_gt[i*8:i*8+3]+np.random.normal(0,mic_pose_std*1,(3,1))#np.zeros_like(x_gt[i*8:i*8+3])#x_gt[i*8:i*8+3]+np.random.normal(0,mic_pose_std*6,(3,1))
        # x[i*8+3:i*8+6] = x_gt[i*8+3:i*8+6]+np.random.normal(0,mic_rot_std*1,(3,1)) #(0,15*np.pi/180,(3,1))#np.zeros_like(x_gt[i*8+3:i*8+6])#x_gt[i*8+3:i*8+6]+np.random.normal(0,mic_rot_std*6,(3,1)) #(0,15*np.pi/180,(3,1))
        # x[i*8+6]       =x_gt[i*8+6]+np.random.normal(0,max_offset/10*1)
        # x[i*8+7]       =x_gt[i*8+7]+np.random.normal(0,max_clock_diff/10*1)
        x[i*8+6]       = 0# np.random.uniform(0,0.05)
        x[i*8+7]       =0# np.random.uniform(0,max_clock_diff)

    # For Experiment 1: x = x_gt + Gaussion Noise
    # for i in range(time_steps):
    #     x[i*3+8*mic_num:i*3+8*mic_num+3] =  x_gt[i*3+8*mic_num:i*3+8*mic_num+3]+ np.random.normal(0,src_pose_std*1,(3,1))
