...
def compare_doa(real,theo):
    doa_diff = real-theo
    dist_norm = norm(doa_diff,axis=1)
    min_index = np.argmin(dist_norm)
    # print("******************cho",min_index)
    return min_index
...
def main():
    ...
    for exp in (exp_time):
        ...
        for i in range(peak_count):
            ...
            master_noise_clip = master_data[zone_master[0] - int(fs*1.5):int(zone_master[0] - fs*0.5), :]
            ...
            server1_noise_clip = server1_data[zone_server1[0] - int(fs * 1.5):int(zone_server1[0] - fs*0.5), :]
            ...
            server2_noise_clip = server2_data[zone_server2[0] - int(fs * 1.5):int(zone_server2[0] - fs*0.5), :]
            ...
            server3_noise_clip = server3_data[zone_server3[0] - int(fs * 1.5):int(zone_server3[0] - fs*0.5), :]

            #  SNR
            S1 = src_pos[i].reshape((-1, 1))
            signal_power_master  = norm(master_sound_clip-
                                        np.nan_to_num(np.mean(master_noise_clip,axis=0), nan=0),
                                        axis=0)
            signal_power_server1 = norm(server1_sound_clip-
                                        np.nan_to_num(np.mean(server1_noise_clip,axis=0), nan=0),
                                        axis=0)
            signal_power_server2 = norm(server2_sound_clip-
                                        np.nan_to_num(np.mean(server2_noise_clip,axis=0), nan=0),
                                        axis=0)
            signal_power_server3 = norm(server3_sound_clip-
                                        np.nan_to_num(np.mean(server3_noise_clip,axis=0), nan=0),
                                        axis=0)

            noise_power_master  = norm(master_noise_clip,  axis=0)
            noise_power_server1 = norm(server1_noise_clip, axis=0)
            noise_power_server2 = norm(server2_noise_clip, axis=0)
            noise_power_server3 = norm(server3_noise_clip, axis=0)

            snr_master = np.mean(20 * np.log10(signal_power_master / noise_power_master))
            pair = np.array([distant(mic_array[0, :3], S1.reshape(-1)), snr_master])
            dist_snr = np.vstack((dist_snr, pair)) if (snr_master > 2 and snr_master < 50) else dist_snr
            snr_server1 = np.mean(20 * np.log10(signal_power_server1 / noise_power_server1))
            pair = np.array([distant(mic_array[1,:3],S1.reshape(-1)),snr_server1])
            dist_snr = np.vstack((dist_snr,pair)) if (snr_server1 > 2 and snr_server1<50) else dist_snr
            snr_server2 = np.mean(20 * np.log10(signal_power_server2 / noise_power_server2))
            pair = np.array([distant(mic_array[2,:3],S1.reshape(-1)),snr_server2])
            dist_snr = np.vstack((dist_snr,pair)) if (snr_server2 > 2 and snr_server2<50) else dist_snr
            snr_server3 = np.mean(20 * np.log10(signal_power_server3 / noise_power_server3))
            pair = np.array([distant(mic_array[3,:3],S1.reshape(-1)),snr_server3])
            dist_snr = np.vstack((dist_snr,pair)) if (snr_server3 > 2 and snr_server3<50) else dist_snr

            total_snr_master.append(snr_master)
            total_snr_server1.append(snr_server1)
            total_snr_server2.append(snr_server2)
            total_snr_server3.append(snr_server3)

            # estimate DOA
            master_doa_angle,master_doa  = doa_calculate(master_sound_clip)
            azi_theo_1, ele_theo_1, doa_theo_1 = azi_ele_calculate(R0, S1, p0)

            index = compare_doa(master_doa,doa_theo_1)
            master_doa = master_doa[index]
            master_doa_angle = master_doa_angle[index]
            ...
            index = compare_doa(server1_doa, doa_theo_2)
            server1_doa = server1_doa[index]
            server1_doa_angle = server1_doa_angle[index]
            ...
            index = compare_doa(server2_doa, doa_theo_3)
            server2_doa = server2_doa[index]
            server2_doa_angle = server2_doa_angle[index]
            ...
            index = compare_doa(server3_doa, doa_theo_4)
            server3_doa = server3_doa[index]
            server3_doa_angle = server3_doa_angle[index]
